$(function () {
    ...
    $("#new_trailer_form").submit(function (e) { 
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#new_trailer_canvas .offcanvas-body').animate({ scrollTop: 0 }, 'slow');
    ...
    var trl_type_val = $("#type_trl_val").val();
    $("#trl_type option[value='"+trl_type_val+"']").prop("selected", true);

    // update trailer information
    $("#update_trailer_form").submit(function (e) { 
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                $("#trl_cancel_btn").show();
                $("#trl_submit_btn").text("Update").attr('type', 'submit');

                var fdback = `<div class="alert alert-${response.success ? 'success' : 'danger'} alert-dismissible fade show px-2 m-0 d-block w-100"><i class='fas fa-${response.success ? 'check' : 'exclamation'}-circle'></i> ${response.sms} <button type="button" class="btn-close d-inline-block" data-bs-dismiss="alert"></button></div>`;
                
                $('#update_trailer_canvas .offcanvas-body').animate({ scrollTop: 0 }, 'slow');
                $("#update_trailer_form .formsms").html(fdback);

                if(response.success) {
                    $("#trailer_basic_info").load(location.href + " #trailer_basic_info");
                }
            },
            error: function(xhr, status, error) {
                console.log(error);
    ...
    $("#confirm_delete_btn").click(function (e) { 
        ...
        formData.append("trailer_id", $("#trailer_update_id").val());
        formData.append("delete_trailer", "delete");

        $.ajax({
            type: 'POST',
            url: $("#update_trailer_form").attr('action'),
            data: formData,
            dataType: 'json',
            contentType: false,
            processData: false,
    ...
    $("#trailers_table thead tr").clone(true).attr('class','filters').appendTo('#trailers_table thead');
    var trailers_table = $("#trailers_table").DataTable({
        fixedHeader: true,
        processing: true,
        serverSide: true,
        ajax: {
        ...
        },
        {
            ...
            createdCell: function (cell, cellData, rowData, rowIndex, colIndex) {
                var cell_content =`<a href="${$("#trailer_url").val()}${rowData.id}/" class="btn btn-bblight btn-sm text-white">View</a>`;
        ...
        buttons: [
            ...
            { // PDF button
                ...
                title: "Trailers - Multiplier ltd",
                filename: 'trailers-multiplier-ltd',
        ...
        initComplete: function() {
            ...
            api.columns([0, 1, 2, 3, 4, 5, 6]).eq(0).each(function (colIdx) {
                ...
                if (colIdx == 1) {
                    ...
                    `<option value="Double tyre">Double tyre</option></select>`;
                    cell.html(select);
    ...
    $("#trailers_search").keyup(function() {
        trailers_table.search($(this).val()).draw();
    });

    $("#trailers_filter_clear").click(function (e) { 
        e.preventDefault();
        $("#trailers_search").val('');
        trailers_table.search('').draw();
    });
});